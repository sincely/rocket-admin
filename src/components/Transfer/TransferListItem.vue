  ...
  >
    ...
    <template v-if="checkableComputed">
      <div class="x-transfer-list-item__checkbox">
        <a-checkbox :checked="checked" :disabled="disabledComputed"></a-checkbox>
      </div>
    </template>
    <!--内容-->
    <div class="x-transfer-list-item__content">
      <slot :checked="checked" :direction="direction" :disabled="disabledComputed" :record="record" name="item">
    ...
    <div class="x-transfer-list-item__extra">
      ...
      <template v-if="isLeftComputed">
        <template v-if="hasChildrenComputed">
          <template v-if="checkableComputed">
            ...
            >
              <a-button :disabled="nextDisabledComputed" size="small" type="text" @click.stop="handleNext">
                下级
              </a-button>
            </a-config-provider>
          </template>
      ...
      <template v-if="isRightComputed">
        <!--删除-->
        <a-button size="small" type="text" @click="handleDelete">
          <template #icon>
